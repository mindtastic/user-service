...
async def add_new_user_settings_by_id(id: str, data: dict):
...
async def retrieve_user_settings_by_id(id: str) -> dict:
    user_settings = await user_settings_collection.find_one({"userId": ObjectId(id)})
    if user_settings:
...
async def update_user_settings_by_id(id: str, data: dict):
...
async def delete_user_settings_by_id(id: str):
    user_settings = await user_settings_collection.find_one({"userId": ObjectId(id)})
    if user_settings:
